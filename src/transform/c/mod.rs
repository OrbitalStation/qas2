            ...
            Some(x) => match x {
                Token::Name(name) => {
                    if let Some(return_type) = Type::rustify(name) {
                            ...
                            Token::LParent => {
                                ...
                                if *it.check()? != Token::RParent {
                                    loop {
                                        ...
                                        let name = it.next()?.expect_value(TokenStr::Name);
                                        result += format!("{}: {}, ", name, Type::rustify(ty).expect("Unknown type")).as_str();
                                        lets.push(Let {
                                            name: name.clone(),
                                            ty: Type::c2id(ty)
                                        });
                                ...
                                loop {
                                    match it.next()? {
                                        Token::Name(x) => {
                                            result.push('\t');
                                            if *x == "return" {
                                                let mut rop = ops::expression(&mut it, &lets).expect("Missing semicolon!");
                        ...
                        if alias != ty { result.push_str(format!("type {} = {};\n", alias, ty).as_str()) }
                    }
                },
